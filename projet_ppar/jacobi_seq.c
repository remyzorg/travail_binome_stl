...
void generateRandomDiagonallyDominantMatrix(double *A, int n) {
  ...
  for (i=0;i<n;i++) {
    for (j=0;j<n;j++) {
      A[i * n + j] = absMax; //randomDoubleInDomain(absMax);
    }
    A[i * n + i] = 3.0; // + randomDoubleInDomain(1.0);
  }
}
...
void generateRandomVector(double *v, int n) {
  ...
  for (i=0;i<n;i++) {
    v[i] = i % 1024;//randomDoubleInDomain(1024.0);
  }
}
...
void printMatrixFocus(double* M, int n, int ifocus, int jfocus) {
    int i, j;

    for(i = 0; i < n; i++) {
        for(j = 0; j < n; j++) {
            printf("%1.2e\t", M[i * n + j]);
            if(i == ifocus && j == jfocus) printf("<-");
        }
        printf("\n");
    }
}
